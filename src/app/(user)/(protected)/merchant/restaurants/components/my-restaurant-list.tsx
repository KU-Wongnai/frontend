...
import MyRestaurantListSkeleton from "@/app/(user)/(protected)/merchant/restaurants/components/my-restaurant-list-skeleton";
      ...
      {loading ? (
        ...
        <div className="flex flex-col justify-center items-center col-span-full p-8">
          ...
          <p className="text-lg text-gray-400 mt-2">
            You currently don&apos;t own any restaurant, let&apos;s add your
            restaurant.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {restaurants.map((restaurant: Restaurant) => (
            <RestaurantCard key={restaurant.id} {...restaurant} isMerchant />
          ))}
        </div>
      )}
    </>
